...
@section('title', ('Restaurant Category List'))
...
<div class="content container-fluid">  <!-- Page Heading -->
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" aria-current="page">{{('Restaurant Category')}}</li>
        </ol>
    </nav>
    ...
    <div class="d-md-flex_ align-items-center justify-content-between mb-2">
        <div class="row">
            <div class="col-md-8">
                <h3 class="h3 mb-0 text-black-50">{{('Restaurant Category')}} {{('List')}}</h3>
            </div>
        </div>
    </div>
                    ...
                    <div class="row flex-between justify-content-between align-items-center flex-grow-1">
                        <div class="col-12 mb-1 col-md-2">
                            <h5 class="flex-between">
                                <div>{{('Restaurant Category Table')}} ({{ $categories->total() }})</div>
                            </h5>
                        </div>
                            ...
                            <form action="{{ url()->current() }}" method="GET">
                                <div class="input-group input-group-merge input-group-flush">
                                    ...
                                    <input id="datatableSearch_" type="search" name="search" class="form-control"
                                           placeholder="{{('Search Category')}}" aria-label="Search orders"
                        ...
                        <div class="col-12 col-md-3">
                            <a href="javascript:" data-toggle="modal" data-target="#category-modal" class="btn btn-primary">
                                ...
                                <span class="text">{{('Add Restaurant Category')}}</span>
                            </a>
                        </div>
                        ...
                        <div id="category-modal" class="modal fade" tabindex="-1" role="dialog"
                            ...
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    ...
                                    <form action="{{route('panel.Category.catstore')}}" method="post" enctype="multipart/form-data">
                                        @csrf
                                        <div class="modal-body">
                                            <div class="row mb-4">
                                                ...
                                                <div class="col-md-8 js-form-message">
                                                    <select name="restaurant_id" id="restaurant_id" class="form-control" required data-toggle="tooltip" title="{{('Add Restaurant') }}">
                                            ...
                                            <div class="form-group" style="margin-bottom:0%;">
                                                <center>
                                                    <img style="width: 150px;border: 1px solid; border-radius: 10px;" id="viewer"
                                                        @if(isset($category))
                                                        src="{{asset('image')}}/{{$category['cat_image']}}"
                                                        @else
                        ...
                        <table id="datatable" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};" class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table" style="width: 100%" data-hs-datatables-options='{
                            ...
                            @foreach($categories as $key=>$c)
                                <tr>
                                    ...
                                    <td>{{$c['cat_name']}}</td>
                                    <td><img class="mb-3" src="{{asset('image')}}/{{$c['cat_image']}}" style="width: 4rem;height: 4rem;"></td>
                                    <td>{{ \Carbon\Carbon::parse( $c->cat_created )->format('d/m/Y') }}</td>
                                    <td>
                                        <label class="toggle-switch toggle-switch-sm" for="stocksCheckbox{{$c->cat_id}}">
                                            <input type="checkbox" onclick="status_change_alert('{{route('panel.Category.status-update',[$c->cat_id,$c->cat_status?0:1])}}', '{{('You want to change this category status')}}', event)" class="toggle-switch-input" id="stocksCheckbox{{$c->cat_id}}" {{$c->cat_status?'checked':''}}>
                                    ...
                                    <td>
                                        ...
                                        href="javascript:" data-toggle="modal" data-target="#edit-modal{{$c->cat_id}}" title="Edit Customer">
                                ...
                                <div id="edit-modal{{$c->cat_id}}" class="modal fade" tabindex="-1" role="dialog"
                                aria-labelledby="exampleModalTopCoverTitle" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        <!-- Header -->
                                        ...
                                        <form action="{{route('panel.Category.catupdate')}}" method="post" enctype="multipart/form-data">
                                            @csrf
                                            <div class="modal-body">
                                                <div class="row mb-4">
                                                    <label for="requiredLabel" class="col-md-4 col-form-label input-label text-md-center">
                                                ...
                                                <div class="row mb-4">
                                                    ...
                                                    <div class="col-md-8 js-form-message">
                                                        <input type="text" class="form-control" name="cat_name" value="{{$c['cat_name']}}" required>
                                                        <input type="hidden" class="form-control" name="cat_id" value="{{$c['cat_id']}}" required>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="row mb-4">
                                                    ...
                                                    <div class="col-md-8">
                                                    <div class="custom-file">
                                                        <input type="file" name="image" id="customFileEg2{{$c['cat_id']}}" class="custom-file-input" accept=".jpg, .png, .jpeg|image/*" onchange="imagechange({{$c['cat_id']}},this)">

                                                        <label class="custom-file-label" for="customFileEg2{{$c['cat_id']}}">{{__('choose')}} {{__('file')}}</label>
                                                ...
                                                <div class="form-group" style="margin-bottom:0%;">
                                                    <center>
                                                        <img style="width: 150px;border: 1px solid; border-radius: 10px;" id="viewer2{{$c['cat_id']}}"
                                                            @if(isset($c['cat_image']))
                                                                src="{{asset('image')}}/{{$c['cat_image']}}"
                                                            @else
                                                                src="{{asset('back-end/assets/back-end/img/900x400/img1.jpg')}}"
                                                            @endif
                                                            alt="image"/>
